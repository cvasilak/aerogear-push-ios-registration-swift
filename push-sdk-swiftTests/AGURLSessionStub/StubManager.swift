...
class StubManager {
    ...
    class var sharedManager: StubManager {

        struct Static {
            static let _instance = StubManager()
        }
    ...
    class func stubRequestsPassingTest(testBlock: StubTestBlock, withStubResponse responseBlock: StubResponseBlock) {
        ...
        StubManager.sharedManager.stubDescriptors += stubDesc
    }
    
    class func addStub(stubDescr: StubDescriptor) {
        StubManager.sharedManager.stubDescriptors += stubDescr
    }
    
    class func removeStub(stubDescr: StubDescriptor) {
        if let index = find(StubManager.sharedManager.stubDescriptors, stubDescr) {
            StubManager.sharedManager.stubDescriptors.removeAtIndex(index)
        }
    }
    
    class func removeLastStub() {
        StubManager.sharedManager.stubDescriptors.removeLast()
    }
    
    class func removeAllStubs() {
        StubManager.sharedManager.stubDescriptors.removeAll(keepCapacity: false)
    }
